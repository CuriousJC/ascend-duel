...
**This skill is for the conversation as much as the plumbing.** "Let's add a relic that does X" is
answered here: which moment it lands on, whether the vocabulary already covers it, and what it
costs if it does not.

## The shape

**A relic is a list of rules.** Each rule is `When` / `If` / `Then`.
...
**`card-damage` and `card-cost` fire per card, and that is the point.** A form relic doubles
...
**Relics fire in the order they are worn**, left to right along the row *(owner's call,
2026-08-17)*. Two relics that both match the same card both apply, the left one first.
...
- **A sold relic's accumulator resets to zero** *(owner's call, 2026-08-21)*. `grown` stays keyed by
  record — a relic put back on is the same relic — but not the same number, so parking a growing relic
  in the shop between fights buys nothing.
- **The shelf is three relics the run is not wearing**, drawn off `seeds.ShopStock`, per fight.

## Discussing a new relic
...
`data/relics.json` and `data/duelist_cards.json` and prints every occupied `(When, Do, If)` cell
with the relics standing in it, then the empty siblings of every filled family. `--gaps` prints
...
### 1. Similarity — is this relic already in the file?

Take the proposal's `(When, Do)` pair and read the grid at it. Three verdicts:

- **Identical** — same moment, same verb, same predicate. Say which relic, and stop; the
