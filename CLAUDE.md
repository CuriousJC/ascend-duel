...
- `internal/entities/` — game-world actors (`Combatant`, embedding `combat.Duelist`), hydrated from `data` records at scene init. **Hydration is a copy now, not a conversion** *(2026-08-16)*: `LifePerCon` went with `Constitution`, so there is no arithmetic left that two callers could do differently. `Combatant.Record` names the roster entry, which is how `internal/decks` finds that enemy's deck; it replaced `Style`.
