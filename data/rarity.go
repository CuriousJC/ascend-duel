...
//
// **Weight and price are deliberately not the same curve.** A rare relic is roughly a tenth as
...
// internal/session, which refuses a misspelled tier rather than shipping a relic that is free and
// never offered.
...
// Price is what the shop charges for a relic of this tier, in vitae. Zero for a tier that does not
// exist, which is a relic that never loads.
func (r Rarity) Price() int { return rarityTiers[r].price }

// Sell is what taking a relic of this tier off pays back.
//
...
//
// **The round trip still loses, and by more the dearer the relic**: 3→1, 5→2, 7→3. A shelf you could
