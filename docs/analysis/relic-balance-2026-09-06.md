...
`data/relics.json` and `data/statuses.json` and can be re-run. See
...
**Four in five shelf seats are an offense relic.** That is not automatically wrong — this is a game
...
**Tempo is the sharpest distortion in the file.** Thirteen relics, and twelve of them are rare:
...
| Payload | Relics | % shelf | What it means |
|---|---:|---:|---|
...
- **Every concept relic is multiplicative, and all fifteen are common.** There is no flat concept
  relic in the game. Given a concept covers four cards where a form covers twelve, a *flat* concept
  relic is the natural cheap version — a big flat number on a narrow trigger — and nobody has
...
**Four relics in the whole game pay out at the run level** — Banker, Hungry, Soul Taker and Heart —
...
the moments where a non-offense relic would naturally live are the emptiest ones in the grammar.

---

## 4. Element parity is close to exact — with one asymmetry

Relics keyed on each element, by category:
...
Forms are flatter — crush, slash and stab are 7 relics each, defend is 3 — and **slash is the only
form with no defense relic at all**, which is the `--holes` output. Given slash cards deal damage
and defend cards raise shields, a defensive slash relic wants a reason to exist before it is
written.
...
1. **`element-elementalist` is the one rung of the eighteen with no relic.** Every other rung has
...
4. **Drawback is two relics.** Onslaught (rare) and Fire of Life (uncommon). The skill names a
...
5. **Tier predicates are five relics** — Atrophy, Erode and Whittle (tempo, one per rung) and
...
1. **Decide whether cost discounts are supposed to be rare-only.** This is the one number that
   ...
   belong at uncommon). Moving three of the element cost relics down to uncommon would take tempo
...
```powershell
python .claude/skills/relic-balance/classify.py            # §1 tables, by rarity
python .claude/skills/relic-balance/classify.py --shelf    # §1 shelf shares
python .claude/skills/relic-balance/classify.py --by payload breadth   # §2
python .claude/skills/relic-balance/classify.py --by category scope    # §3
python .claude/skills/relic-balance/classify.py --by category build    # §4
python .claude/skills/relic-balance/classify.py --holes    # §5
```
