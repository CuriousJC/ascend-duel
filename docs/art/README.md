...
| `background_art_prompt.MD` | the backdrop a duel is fought in front of — a 1920x1080 **JPEG**, one per record in `data/backgrounds.json`. It is the one prompt that is a *place* rather than an object, and it carries the map of what the combat screen lays over the picture, the rule that the room is big enough to hold a duel, and the rule that nobody is in it. Filed by hand into `assets/background/` under the record's `Art` stem, globbed and keyed by stem; there is no `tools/relicart` kind for it yet |
| `bar_art_prompt.MD` | the three cells a resource bar is built from — empty, spent, over — each two end caps and a middle every column of which is identical, so the game stretches only the middle. Used by the AP bar and the round timer, through `ui.DrawBarCell`. Filed by hand into `assets/bar/` |
