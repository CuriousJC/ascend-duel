...
//
// **It is bounded, and it used to claim a bound it no longer had.** The comment here read "60
// cards times two sizes times three states" — 360 — and that was true when a Spec was a name, a
// cost, an element and a state. A Spec carries twenty-odd fields now: a mark bitmask that
// composes, an upgrade, a rarity, four independent booleans, a counter, a shield count and a
// row of colored text spans, every one of which is part of the key. The key space is the product
// of all of them, so nothing about the deck bounds it, and each entry is an `*ebiten.Image` the
// map holds a live reference to — which is exactly what stops the GC ever reclaiming one.
//
// **FIFO rather than LRU, because the cap is far above the working set.** What is on screen at
// once is at most the deck overlay plus a hand plus a relic row, in a handful of states; evicting
// the oldest entry cannot reach one of those, so the cheaper policy is also the correct one here.
// If a profile ever shows re-renders during play, that is the signal the cap is too low — not the
// signal to write an LRU.
var (
	cardCache = map[cardKey]*ebiten.Image{}

	// cardOrder is insertion order, so the oldest entry is cardOrder[0]. It is a slice rather than
	// a ring because eviction happens one entry at a time on insert and never in a burst.
	cardOrder []cardKey
)

// maxCardCache is how many rendered faces are kept.
//
// **Generous on purpose.** Rendering a card writes every pixel in Go, so an eviction that reaches
// a card still on screen costs a visible repaint; the cap is several times the largest plausible
// working set so that cannot happen, and its job is only to stop a long session growing without
// limit. A full card at 162x224 is about 145 KB of texture, so this is a ceiling in the tens of
// megabytes rather than a tight budget.
const maxCardCache = 512

// rememberCard files a rendered face and evicts the oldest if the cache is full.
//
// **Deallocate is called on the way out.** The map dropping its reference is enough for the GC to
// reclaim the image eventually; Deallocate releases the texture now, which is the whole point of
// having a bound. It is documented as safe on an image something else still holds — such an image
// simply reallocates on next use — so a face drawn earlier this frame cannot be broken by it.
func rememberCard(key cardKey, img *ebiten.Image) {
	cardCache[key] = img
	...
	if evicted := cardCache[oldest]; evicted != nil {
		evicted.Deallocate()
	}
...
// than either alone.
func cardBadge(c combat.Card) string {
...
func cardBadgePct(c combat.Card) int {
	if c.Spec().Verb == combat.VerbAttack {
		return c.Amount()
	}
	return 0
...
func cardShields(c combat.Card) int {
...
func HeldByRun(gs *state.GlobalState, c combat.Card) held {
	if gs.Run == nil {
...
func CardImage(gs *state.GlobalState, spec cards.Spec, st cards.Style) *ebiten.Image {
	...
	if err != nil {
		log.Println(err)
		rememberCard(key, nil) // negative-cached, so a broken card logs once and not per frame
		return nil
	}

	img := ebiten.NewImageFromImage(rendered)
	rememberCard(key, img)
	return img
}
