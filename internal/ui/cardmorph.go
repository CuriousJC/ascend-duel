...
//
// An essence eats a card's element and hands back a different card. A rune overwrites one. A relic
...
// Replaces reports whether this morph is one finished face turning into another, rather than a
// card arriving out of nothing or being eaten away.
//
// **A method rather than two exported booleans**, because what a caller wants is the distinction
// MorphInto / MorphIn / MorphAway already draw: holding both faces *is* being a replacement. Two
// fields would let somebody outside this package invent a fourth combination that means nothing.
func (m Morph) Replaces() bool { return m.hasBefore && m.hasAfter }

// Clock is this morph's travel, so a caller can check that two of them are running together.
//
// **Read-only by value.** The clock is the morph's own state and ticking a copy advances nothing,
// which is what makes handing one out safe; what a caller legitimately wants is to compare two.
func (m Morph) Clock() Travel { return m.t }

// Eats reports whether this morph takes a card away to nothing, and Arrives whether it brings one
// out of nothing. With Replaces they are the three shapes MorphInto, MorphAway and MorphIn build,
// and they are exactly what a caller outside this package may ask.
