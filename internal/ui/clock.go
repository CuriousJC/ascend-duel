...
const (
	...
	//
	// **Only the scripted demo reads it**, which is why a bare `staticcheck ./...` calls it unused:
	// that run does not compile the demoplay file. A symbol is dead only when it is dead under
	// every build tag.
	TicksPerSecond = 60
