...
var sheets = []sheet{
	...
	{
		Dir:   "marksheet",
		Tool:  "./tools/marksheet",
		Title: "Mark sheet",
		Blurb: "Every form mark and every cost tick, as a matrix: four silhouettes across, five " +
			"elements and a neutral down, at both sizes the game draws them and enlarged. " +
			"**The question is the matrix rather than the picture** — a form that cannot be told " +
			"from its neighbour and an element that cannot be told from its neighbour are both " +
...
func run(dir string, indexOnly bool) error {
	...
	if err := tmpl.Execute(f, index{Sheets: sheets}); err != nil {
